...
static void
  SsThreshChange (Ptr<OutputStreamWrapper> stream, uint32_t oldCwnd, uint32_t newCwnd)
  {
    NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t" << newCwnd);
    *stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
  }
...
TraceSsThresh(std::string ssthresh_tr_file_name)
{
  ...
  if (ssthresh_tr_file_name.compare("") == 0)
  ...
  else
    {
      ...
      Config::ConnectWithoutContext ("/NodeList/1/$ns3::TcpL4Protocol/SocketList/0/SlowStartThreshold",MakeBoundCallback (&SsThreshChange, stream));
    }
}
...
int main (int argc, char *argv[])
{
  ...
  bool tracing = true;
  std::string tr_file_name = "traceFile";
  std::string cwnd_tr_file_name = "cwndTraceFile";
  std::string ssthresh_tr_file_name = "ssthreshTraceFile";
