...
//  LogComponentEnable ("InterferenceHelper", LOG_LEVEL_ALL);
  ...
  NetDeviceContainer devices1 = wifi.Install (wifiPhy, wifiMac, c.Get (3));
  wifiPhy.Set ("TxGain", DoubleValue (offset + Prss) );
  devices1.Add (wifi.Install (wifiPhy, wifiMac, c.Get (2)));
  ...
  NS_LOG_INFO ("Assign IP Addresses.");
  ipv4_1.SetBase ("10.1.1.4", "255.255.255.0");
  Ipv4InterfaceContainer j = ipv4_1.Assign (devices);

  Ptr<Socket> recvSink1 = Socket::CreateSocket (c.Get (3), tid);
  InetSocketAddress local1 = InetSocketAddress (Ipv4Address ("10.1.1.4"), 81);
  ...
  InetSocketAddress remote1 = InetSocketAddress (Ipv4Address ("255.255.255.255"), 81);
  source1->SetAllowBroadcast (true);
  source1->Connect (remote1);

  // Tracing
  wifiPhy.EnablePcap ("wifi-simple-interference", devices.Get (0));
   wifiPhy.EnablePcap ("wifi-simple-interference", devices.Get (3));
  ...
  Simulator::ScheduleWithContext (source1->GetNode ()->GetId (),
                                  Seconds (startTime), &GenerateTraffic,
                                  source1, PpacketSize, numPackets, interPacketInterval);
